...
| `MCP_TOOL_PROFILE`         | Vercel only       | `full` by default (all tools); set to `memory` to expose only the fact/thought memory tools: recall and search, `remember_fact` and `capture_thought`, retract/restore, and the destructive `forget_thought`, `forget_fact`, and `forget_entity`. The authoritative list is `MCP_MEMORY_TOOL_NAMES` in `apps/web/src/lib/mcp/tool-policy.ts` |
